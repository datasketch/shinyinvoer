...
server <- function(input, output, session) {
  ...
  output$input_1 <- renderUI({
    spectrumColorPicker('input_id_1', "New palette. Limited to 3 colors",
  ...
  output$input_2 <- renderUI({
    spectrumColorPicker('input_id_2', "Predefined palette",
  ...
  output$input_3 <- renderUI({
    spectrumColorPicker('input_id_3', "Will be updated", colors = c('#264653', '#2a9d8f', '#e9c46a'))
  })
  ...
  output$input_4 <- renderUI({
    spectrumColorPicker('input_id_4', "Palette update", palette = c('#606c38', '#283618', '#fefae0', '#dda15e', '#bc6c25'))
  })
  ...
  observe({
    if (input$input_3_update) {
      updateSpectrumColorPicker(session, inputId = 'input_id_3', colors = c('#e63946', '#e5989b'))
      # If you want a fresh start you can update without any colors
      # updateSpectrumColorPicker(session, inputId = 'input_id_3')
    }
    if (input$input_4_update) {
      updateSpectrumColorPicker(session, inputId = 'input_id_4',
